...
public class DA_TransactionHistory
{
    ...
    > GetTransactionHistoryListByAccountNoAsync(string accountNo)
    {
        ...
        try
        {
            ...
            IEnumerable<TransactionDataModel> dataLst = await db.QueryAsync<TransactionDataModel>(
            ...
            var model = new TransactionHistoryListResponseModel { DataLst = lst };
    ...
    )
    {
        ...
        try
        {
            ...
            var fromAccount = await _appDbContext
            ...
            var toAccount = await _appDbContext
                .Accounts.AsNoTracking()
            ...
            var transactionHistory = await _appDbContext
                .TransactionHistories.AsNoTracking()
                .Where(x =>
                    x.TransactionDate == today && x.FromAccountNo == requestModel.FromAccountNo
                )
                .ToListAsync();
            ...
            if (fromAccLevel == 1m)
            {
                ...
                {
                    responseModel = Result<TransactionResponseModel>.FailureResult(
                        "You exceed the transfer limit for today."
                    );
                    goto result;
                }
            }
            ...
            if (
                fromAccountSavingResult > 0
                && toAccountSavingResult > 0
                && transactionHistorySavingResult > 0
            )
            {
